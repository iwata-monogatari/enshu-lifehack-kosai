...
湖西市公式ページ本文と自動突合する機械検証スクリプト。

方針（袋井版で確定した全市共通ルールを継承）:
  湖西市役所およびWEBから取れる事実だけを掲載する。
...
UA = 'enshu-lifehack-verify/1.0 (+https://kosai.enshu-lifehack.com/)'
WAIT = 0.5
KAKUNIN = '確認中'
...
def main():
    ...
    for item in targets:
        ...
        for s in item.get('sources_kosai', []):
            url = s['url']
        ...
        if all_url_ok and not fact_ng:
            ...
            ok_items += 1
            lines.append(f"- ✅ machine-verified（URL {len(item.get('sources_kosai', []))}本・facts一致 {len(fact_ok)}件）")
        else:
            mismatch_items += 1
        lines += notes + ['']
