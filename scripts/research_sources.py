...
# -*- coding: utf-8 -*-
"""
research_sources.py — 湖西市公式サイトをクロールしてページインベントリを作り、
台帳(topics_master.json)のdraft項目に出典URL候補を自動マッチングする。

工程（袋井版で確立した「AI下調べ」の機械化方針を継承）:
  1) クロール: www.city.kosai.shizuoka.jp をトップからBFSで巡回し、
     全HTMLページの URL・<title>・見出し(h1) を data/kosai_pages.json に保存。
  ...
  2) マッチング: 各draft項目の (title_iwata + synonyms + 磐田側ラベル語) と
     ...
     上位候補(スコア閾値以上・最大4本)を sources_kosai に登録 → status: ai-checked。
...
BASE = 'https://www.city.kosai.shizuoka.jp'
LEDGER = Path('data/topics_master.json')
INVENTORY = Path('data/kosai_pages.json')
REPORT_DIR = Path('reports')
UA = 'enshu-lifehack-research/1.0 (+https://kosai.enshu-lifehack.com/)'
WAIT = 0.5
MAX_PAGES = 6000
...
STOP = set(('湖西市 磐田 磐田市 について ご案内 案内 一覧 情報 お知らせ ページ ホーム サイト 公式 '
